...
def Visualize_Image(net,path,img_size,save_path,scale_factor=1,out_threshold=0.5):
    ...
    for x in range(0, big_img.shape[1] - size, size):
        for y in range(0, big_img.shape[0] - size, size):
            ...
            # )
            mask_pred = F.interpolate(mask_pred, (size, size), mode='bilinear')
            mask_pred = torch.sigmoid(mask_pred) > out_threshold
...
if __name__ == '__main__':
    ...
    Visualize_Image(model,path,img_size,save_path,scale_factor=1,out_threshold=0.5)